...
{
  /// \brief Save an image
  /// \param[in] _data the temperature data to be saved
  ...
  public: bool SaveThermalImage(const uint16_t *_data, unsigned int _width,
    unsigned int _height);

  /// \brief Helper function to convert temperature data (uint16_t) to float32
  /// \param[in] _data temperature data in uint16_t format
  /// \param[out] _tempBuffer resulting float32 temperature data in Kelvin
  /// \param[in] _width width of image
  /// \param[in] _height height of image
  /// \param[in] _minTemp minimum temperature
  /// \param[in] _maxTemp maximum temperature
  /// \param[in] _resolution linear resolution
  public: bool ConvertTemperatureToFloat32(const uint16_t *_data,
    float *_tempBuffer, unsigned int _width, unsigned int _height,
    float _minTemp, float _maxTemp, float _resolution);
  ...
  /// \brief Thermal data buffer used when saving image (float32).
  public: float *imgThermalBuffer = nullptr;
  ...
  const std::chrono::steady_clock::duration &_now)
{
  ...
  *stamp = msgs::Convert(_now);
  ...
  // Save thermal image as float32 TIFF
  if (this->dataPtr->saveImage)
  {
    this->dataPtr->SaveThermalImage(this->dataPtr->thermalBuffer, width, height);
  }
...
//////////////////////////////////////////////////
bool ThermalCameraSensorPrivate::ConvertTemperatureToFloat32(
    ...
    float _minTemp, float _maxTemp, float _resolution)
{
  ...
  // Convert each uint16_t value to float32 temperature in Kelvin
  // Formula: temperature = minTemp + (uint16_value * resolution)
  for (unsigned int i = 0; i < numPixels; ++i)
  {
    _tempBuffer[i] = _minTemp + (static_cast<float>(_data[i]) * _resolution);
  }
...
//////////////////////////////////////////////////
bool ThermalCameraSensorPrivate::SaveThermalImage(const uint16_t *_data,
    unsigned int _width, unsigned int _height)
{
  ...
  // Allocate or reallocate float32 buffer if needed
      ...
      static_cast<int>(_height) != this->imgThermalBufferSize.Y())
  {
    ...
    unsigned int bufferSize = _width * _height;
    this->imgThermalBuffer = new float[bufferSize];
    this->imgThermalBufferSize = math::Vector2i(_width, _height);
  }

  // Convert uint16_t temperature data to float32
  this->ConvertTemperatureToFloat32(_data, this->imgThermalBuffer,
      _width, _height, this->minTemp, this->maxTemp, this->resolution);

  // Create filename
  std::string filename = this->saveImagePrefix +
                         std::to_string(this->saveImageCounter) + ".tiff";
  ++this->saveImageCounter;

  // Create common::Image from float32 data and save as TIFF
  common::Image localImage;
  localImage.SetFromData(this->imgThermalBuffer, _width, _height,
      common::Image::FLOAT32);
  localImage.SaveTIFF(
      common::joinPaths(this->saveImagePath, filename));

  gzdbg << "Saved thermal radiometric TIFF: " << filename << std::endl;
